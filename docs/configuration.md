...
| `staleness_hours`          | `24`               | How old the last base fetch may get before `wt new`/`wt claim` refetch it (one notice, `--no-fetch` to skip) and `wt ls` marks it stale. Never triggers a network call in `wt ls`. See [pool mode → staying fresh](pool-mode.md#staying-fresh). |
