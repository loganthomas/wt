...
func TestDetectDefaultsEmptyRepo(t *testing.T) {
	...
	if d.marker != "" || d.refresh != "" || d.copies != nil {
...
func TestDetectDefaultsMostSpecificLockfileWins(t *testing.T) {
	...
	if d.refresh != "pnpm install" || d.marker != "pnpm-lock.yaml" {
		t.Errorf("proposed %q gated on %q, want pnpm install on its lockfile", d.refresh, d.marker)
	}
}
